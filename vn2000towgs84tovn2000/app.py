...
# ==== 1. CẤU HÌNH TRANG VÀ HÀM BACKGROUND ====
...
with st.sidebar:
    ...
    st.markdown(f"- 🔍 **Lượt truy cập:** `{visit_count}`")
    st.markdown(f"- 👍 **Lượt thích:** `{like_count}`")
    if st.button("👍 Thích ứng dụng này"):
        like_count += 1
...
with tab1:
    st.markdown("#### 🔢 Nhập tọa độ VN2000 (X Y Z – cách nhau bởi space/tab/newline, có thể kèm STT):")
...
with tab2:
    st.markdown("#### 🔢 Nhập tọa độ WGS84 (Lat Lon H – cách nhau bởi space/tab/newline, có thể kèm STT):")
...
if "df" in st.session_state:
    ...
    # Xuất KML nếu là kết quả WGS84
    kml_str = df_to_kml(df)
    if kml_str:
        st.markdown("### 📥 Xuất file KML")
